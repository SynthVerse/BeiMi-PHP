...
use app\common\service\pay\PayNoticeService;
...
class PayController extends BaseApiController
{
    ...
    public array $notNeedLogin = ['notifyMnp', 'notifyOa', 'aliNotify', 'jxgNotify'];
     ...
     * @date 2024/3/18 16:50
     */
    public function jxgNotify()
    {
        $paramData = $this->request->post();
        if ($paramData['trade_status'] == 'TRADE_SUCCESS' && $paramData['code'] == '10000') {
            $orderNum = $paramData['out_trade_no'];
            $transactionId = $paramData['trade_no'];
            $openid = $paramData['openid'];
            $totalFee = $paramData['total_amount'];
            $from = isset($paramData['extra_common_param']) ? $paramData['extra_common_param'] : "";
            app()->make(PayNoticeService::class)->notify($orderNum, $transactionId, $totalFee, $from);
            echo 'success';
            exit;
        }
    }
