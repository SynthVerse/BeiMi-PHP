...
class RechargeLists extends BaseAdminDataLists implements ListsSearchInterface, ListsExcelInterface
{
    ...
    {
        return [
            '=' => ['ro.pay_sn', 'ro.pay_way', 'ro.pay_status'],
        ];
    }
    ...
    public function queryWhere()
    {
        ...
        if (!empty($this->params['user_info'])) {
            $where[] = ['u.nickname|u.mobile|u.real_name', 'like', '%' . $this->params['user_info'] . '%'];
        }
    ...
    {
        $field = 'ro.id,ro.pay_sn,ro.order_amount,ro.pay_way,ro.pay_time,ro.pay_status,ro.create_time,ro.refund_status';
        $field .= ',u.real_name,u.nickname,u.account';
        $field .= ',p.name as pay_name';
        $lists = RechargeOrder::alias('ro')
            ->join('user u', 'u.id = ro.user_id')
            ->join('tenant_pay_config p', 'p.id = ro.pay_way')
            ->field($field)
