...
class PayWayLogic extends BaseLogic
{
    ...
    public static function getPayWay()
    {
        ...
        for ($i = 1; $i <= max(array_column($payWay, 'scene')); $i++) {
            foreach ($payWay as $val) {
                if ($val['scene'] == $i) {
                    $val['pay_way_name'] = TenantPayConfig::where('id',$val['pay_config_id'])->value('name');
    ...
    public static function setPayWay($params)
    {
        ...
        $payWayList = TenantPayWay::select()->toArray();
        $payWayListArr = array_column($payWayList, null,'id');
        $data = [];
        foreach ($params as $key => $value) {
            ...
            foreach ($value as $val) {
                ...
                if(!isset($payWayListArr[$val['id']])){
                    continue;
                }

                if ($payWayListArr[$val['id']]['is_default'] != $val['is_default'] || $payWayListArr[$val['id']]['status'] != $val['status']) {
                    TenantPayWay::where('id', $val['id'])->update([
