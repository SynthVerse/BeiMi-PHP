> **旧副本提示**：本文件位于 `BeiMi-PHP/docs/`，可能落后于权威根目录文档。请优先查看 `E:\object\BeiMi\docs\backend-implementation-plan.md`，后续仅按需同步关键提示。
...
> **校正日期**：2026-04-20  
> **实施状态**：Phase 1-6 已完成，共 14 张表、81 条路由、16 个 Logic/Service 类  
> **最新修复**：销售单表追加 from_purchase_order_id 字段（迁移 20260420_000001）  
> **已验证能力**：多租户隔离、库存财务联动、bcmath 精度、订货单状态机、端到端业务闭环
...
| 列表格式 | `{code:1, data:{data:[], total:N, page:N, pagesize:N}}`，已在 BaseJxcController 完成适配 |
...
- 数据库：新建 14 张业务表（含 3 张流水表 lk_stock_flow / lk_receivable_flow / lk_payable_flow）
...
| 7 | 进货单（供货单） | 6 | `api/supply.js` | `/api/supply` |
...
#### Phase 6：联调测试优化（预计 3 天）**[已完成 2026-04-20]**
...
├── PurchaseOrder.php
├── OrderGoods.php
├── StockFlow.php          (Phase 2 新增)
├── ReceivableFlow.php     (Phase 2 新增)
├── PayableFlow.php        (Phase 2 新增)
└── AuditLog.php           (Phase 3 新增)
```
