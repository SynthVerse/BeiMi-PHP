...
- 客户门店绑定、解绑、下属门店列表。
- 客户汇总、搜索、应收汇总接口的基础形态。
- 客户销售历史目前仍是占位返回，待销售单链路进一步接入真实历史。
...
- 当前客户销售历史仍是占位返回。
- 需要基于 `sales_order.customer_id` 补真实分页列表。
- 需要考虑总客户与门店客户的聚合关系。
...
OpenAI 集成目前只完成了工具包和代理准备，尚未进入业务代码实现。
...
- 订单类接口尚未加入幂等控制，用户连续点击可能产生重复订单。
- OpenAI 相关功能接入后需要额外处理提示词注入、敏感信息外发和日志脱敏。
...
项目已经从“规划和接口反推阶段”进入“真实后端模块落地和联调阶段”。
...
更稳妥的路线是先把接口契约、自动化冒烟、库存流水和应收应付模型补齐，再按“进货单 → 销售退货单 → 订货单 → OpenAI 文本识别”的顺序推进。这样可以减少返工，把真实阻塞点转化为可复用的团队经验，而不是在每个新模块里重复踩坑。
